...
os.chdir('data_and_figures/xyz_floquet')

# Load the datasets
exact_data = np.load('exact_xyz_floquet_params=1_1_1_0_bc=open.npz')
trotter_data = np.load('trotter_evolution_n=20_xyz_floquet_n_qubits=8_params=1_1_1_0_bc=open.npz')
with open('adaptive_pvqd_xyz_floquet_params=1_1_1_0_bc=open_tol=0.9999.pkl', 'rb') as f:
...
pvqd_x0, pvqd_z0, pvqd_x0x1, pvqd_z0z1 = np.array(adaptive_pvqd_data["observables values"]).T
...
# Top panel to plot <x0x1>
ax[0].scatter(pvqd_t, pvqd_x0x1, s=8, marker='o', c=colors1[0], linewidths=0.8, edgecolors=colors1[1])
ax[0].scatter(trot_t, trotter_data["x0x1"], s=8, marker='s', c=colors2[0], linewidths=0.8, edgecolors=colors2[1])
ax[0].plot(exact_data['times'], exact_data['x0x1'], ls='--', linewidth=1, color='k')
ax[0].set(ylabel='$\\langle \sigma_0^x \sigma_1^x\\rangle$')
ax[0].set_xlim(xmin=0, xmax=2)

# Middle panel to plot <x0x1>
ax[1].scatter(pvqd_t, pvqd_z0z1, s=8, marker='o', c=colors1[0], linewidths=0.8, edgecolors=colors1[1])
ax[1].scatter(trot_t, trotter_data["z0z1"], s=8, marker='s', c=colors2[0], linewidths=0.8, edgecolors=colors2[1])
ax[1].plot(exact_data['times'], exact_data['z0z1'], ls='--', linewidth=1, color='k')
ax[1].set(ylabel='$\\langle \sigma_0^z \sigma_1^z \\rangle$')
...
ax[0].legend(handles=[a02,(a20,a22),(a21,a23)], labels=['Exact','Adaptive pVQD','Trotter ($n=20$)'],
            loc='upper center', bbox_to_anchor=(0.5, 1.3), ncol=3, fontsize=8,
            handler_map={tuple: HandlerTuple(ndivide=None)})

plt.tight_layout()
# plt.savefig("adaptive_pvqd_xyz_floquet_n_qubits=8_params=1_1_1_0_bc=open_tol=0.9999.pdf", dpi=600)
